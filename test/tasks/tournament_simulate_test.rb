...
class TournamentSimulateTest < ActiveSupport::TestCase
  ...
  test "assign_simulation_points is a no-op (scoring derived from match results)" do
    friend = Friend.create!(name: "GS_#{SecureRandom.hex(4)}")
    group  = Group.create!(name: "GS_Group_#{SecureRandom.hex(4)}", friend: friend)
    ...
    # Should not raise
    assert_nothing_raised { TournamentSimulation.assign_simulation_points(match) }
  end

  test "simulate_knockout_match creates a PostEvent match with a winner" do
    friend = Friend.create!(name: "KO_#{SecureRandom.hex(4)}")
    group  = Group.create!(name: "KO_Group_#{SecureRandom.hex(4)}", friend: friend)
    ...
    # Scoring is now computed from match results via progression_score
    assert home.reload.progressed? || away.reload.progressed?, "at least one team should be marked as progressed"
  end
  ...
  def build_simulation_data
    12.times do |g|
      ...
      group  = Group.create!(name: "SimGroup#{g}", friend: friend)
      4.times do |t|
  ...
  test "simulate task creates 72 group stage matches and selects 16 qualifiers" do
    ...
    BenBotcurdyService.stub(:new, stub_commentary) do
      STDIN.stub(:gets, "yes\n") do
